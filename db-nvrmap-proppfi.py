...
parser = argparse.ArgumentParser(description='Process View PFIs to an Ensym'
                                 ' shapefile.')
parser.add_argument('view_pfi', metavar='N', type=int, nargs='+',
                    help='PFI of the Parcel View')
parser.add_argument('-p', '--property', action='store_true', 
                    help='use PFI of the Property View instead of Parcel View')
parser.add_argument("-s", "--shapefile", default='nvrmap',
...
if args.property:
    ...
    # Define the SQL query for property_detail pfi
    sql_query = f"""
    WITH parcel_pfi_from_pr_pfi AS (
        SELECT parcel_pfi FROM parcel_property
        INNER JOIN property_detail
            ON parcel_property.pr_pfi = property_detail.pfi
        WHERE property_detail.pfi = '45437823'
    )

    SELECT parcel_detail.view_pfi FROM parcel_detail
    INNER JOIN parcel_pfi_from_pr_pfi
    ON parcel_pfi_from_pr_pfi.parcel_pfi = parcel_detail.pfi
    """

    view_pfi_df = pd.read_sql(sql_query, con=engine.connect())
    view_pfis = view_pfi_df['view_pfi'].to_numpy()
    print(view_pfis)


# Define the SQL query for parcel_view pfi
sql_query = f"""
SELECT bio_clipped.*
FROM (
